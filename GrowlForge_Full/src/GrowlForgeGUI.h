...
void drawDynamicUi(Graphics&g,GuiState*ui,const RectF&dirty){
 ...
 if(rectIntersects(dirty,autoGainRect)){
  const RectF ag(881.0f,510.0f,82.0f,38.0f);
  drawToggleDynamic(g,ui,ag,ui->owner->p[AutoGain].load()>=0.5,false,ui->hover==AutoGain);
 }
 ...
 if(rectIntersects(dirty,commitRect)){
  ...
  drawText(g,ui,correction,commit,9.5f,ui->owner->p[AutoGain].load()>=0.5?colorTeal():colorMuted(),FontStyleBold);
 }
...
void timerTick(GuiState*ui){
 ...
 for(const auto&k:kKnobs){
  const double value=ui->owner->p[k.id].load();
 ...
 for(const clap_id id:{X2,AutoGain,AutoGainCorrection}){
  const double value=ui->owner->p[id].load();
 ...
 const double displayValue=ui->owner->p[displayId].load();
